...
zstyle ':vcs_info:git:*' formats       '%s: %r on %b%m%u%c'
zstyle ':vcs_info:git:*' actionformats '(%b|%a%u%c)'


PROMPT='%F{red}[%f%F{214}%n%f%F{red}@%f%F{010}%m%f %F{014}%B%1~%b%f%F{red}]%f 
{%h}  %# %F{green}>%f '
RPROMPT='%F{227}${vcs_info_msg_0_}%f %F{red}[%f%w - %T%F{red}]%f'
# PROMPT='%n@%m %1~ %#'
alias la='ls -a'
#alias cdtp='cd /Users/xavier2p/OneDrive\ -\ EPITA/tp_info/S2/abacus_project; pwd; ls -a'
